...
def run_mat_code(eng, trip_i, trip_j, case_name, verbose=False, print_opt=False):
    print()
...
if (trip_i, trip_j) in failure_pairs:
    print("pair ({}, {}) is unable to finish the computation.".format(trip_i, trip_j))
    exit()

num_failure_pairs = get_num_failure_pairs(trip_i, trip_j, failure_pairs, num_branches)
...
print("> Total Number of Generated Samples: {}".format(num_samples * num_outage_pairs))
...
if num_outage_pairs == 1:
    run_mat_code(eng, trip_i, trip_j, case_name, verbose, print_opt)
